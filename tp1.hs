...
eval env (Dcall dexp1 dexp2) =
    case(dexp1, dexp2)of
        (Dlambda n , Dnum m) -> Vnum m
        (Dlambda n , _ ) -> eval env dexp2
        -- ((Dcall n m),_) -> 
        --     let
        --         (Vfun val) = eval env (Dcall n m)
        ...
        (_,_) ->
            ...
            in
                val evalDexp
eval env (Dlambda dexp) = 
    case dexp of
        (Dnum n) -> Vnum n
        (Dref s) -> env !! s
        (Dcall m n) -> eval env (Dcall m n)
eval env (Dadd dexp1 dexp2) = Vcons(eval env dexp1)(eval env dexp2)
    ...
    --     case (fun, lexp)of
    --         ((var:env), Lnum n) -> Vnum n
    --         (Lref n) -> env !! n
    --         let
    --             fun = Vfun (\ (Vnum n) -> Vnum n)
    --             expandEnv = ((env!!n, fun): env)
    --     -- (Dcall v1 v2)-> 
    --     --     let 

    -- --     -- (_) ->  Vfun (\ (Vnum x) -> Vfun (\ (Vnum y) -> Vnum (x + y))))
...
--  ("x", Vfun (\ (Vnum x) -> Vnum (x)))
-- ("y", Vfun (\ (Vnum y) -> Vnum (y)))
-- ("x", Vfun (\ (Vnum x) -> Vfun (\ (Vnum y) -> Vnum (x - y))))

--  (((fn (x) (fn (y) (* x y))) 3) 5) 
main = print(valOf "((fn(x) x) 2)")
-- Lcall (Llambda "x" (Lref "x")) (Lnum 2)
--Dcall (Dlambda (Dref 0)) (Dnum 2)
-- main = print(dexpOf "(add (add 1 2) 3)")
-- Lcall (Lcall (Lref "+") (Lref "x")) Lnil
-- Dcall(Dcall(Dref 0)(Dref "X"))
-- Scons (Ssym "+") (Scons (Ssym "x") Snil)
